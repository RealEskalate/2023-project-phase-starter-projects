...
const AccountSetting = () => {
  ...
  const canSaveChange = Boolean(currPassword) && Boolean(newPassword) && Boolean(confirmPassword)

  const [editPassword, { isSuccess, isError, isLoading }] =
      ...
      <div className="flex flex-col items-center md:items-stretch space-y-4 py-8 border-b md:py-12 md:space-y-2">
        <div className="flex justify-between">
          ...
          disabled={!canSaveChange}
            ...
            className="hidden px-8 py-4 text-xs text-white font-semibold rounded-lg disabled:bg-gray-500 bg-primary-color md:text-sm md:block"
          >
            Save Changes
          </button>
        </div>
        ...
        <button
          onClick={() => onSubmitHandler()}
          className="px-5 py-4 text-xs text-white font-semibold rounded-lg bg-primary-color md:hidden"
        >
          Save Changes
        </button>
      </div>
      <form className={`flex flex-col justify-center items-center py-12 w-full ${isLoading && "animate-pulse"}`}>
        <div className="md:flex md:items-center mb-6 w-1/2">
          ...
          <div className="md:w-1/2 relative">
            ...
            {showCurrPassword && (
              <IoMdEye
                className="absolute top-1/3 right-3.5 w-6 h-6 text-[#B7B7B7] cursor-pointer"
